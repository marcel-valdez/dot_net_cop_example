namespace Game.Core.Impl
{
    ...
    internal class Log : ILog
    {
        ...
        public bool AddToLog(string message)
        {
            ...
            {
                filepath = string.Format(@"{0}\", AppDomain.CurrentDomain.BaseDirectory);
            }
            else
            {
                filepath = @"";
            ...
            try
            {
                ...
                lock (Log.LogLock)
                {
                    ...
                    using (FileStream xmlFile = new FileStream(filepath + filename, FileMode.Append, FileAccess.Write))
                    {
                        using (StreamWriter writer = new StreamWriter((Stream)xmlFile))
                        {
                            writer.Write(line);
                        }
                    }
