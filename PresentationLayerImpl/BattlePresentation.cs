namespace Game.Presentation.Impl
{
  ...
  internal class BattlePresentation : IBattlePresentation
  {
    ...
    private void InitCommonState()
    {
      ...
      this.mMyCards = this.player.Deck.Cards
                                 .Select(c => new BattleCardPresentation(c))
                                 .ToList();
      this.mOppCards = this.opponent.Deck.Cards
                                 .Select(c => new BattleCardPresentation(c))
                                 .ToList();
       
      /// A player should not be able to select his opponents cards.
      foreach (BattleCardPresentation card in this.mOppCards)
      {
        card.Selectable = false;
      }
      
      /// A player should be able to select his cards.
      foreach (BattleCardPresentation card in this.mMyCards)
      {
