namespace Game.Presentation.Tests
{
  ...
  public class TestIBattlePresentation
  {
    ...
    public void IsCorrectlyInitialized()
    {
      ...
      // Assert
      Verify.That(target.ConfirmBtnEnabled)
            .IsTrue().Now();
      Verify.That(target.HarmResultsVisible)
            .IsFalse().Now();
      Verify.That(target.Title)
            .ItsTrueThat(msg => !string.IsNullOrEmpty(msg)).Now();
      Verify.That(target.MiddleMsgTitle)
            .ItsTrueThat(msg => string.IsNullOrEmpty(msg)).Now();
      Verify.That(target.MyCards).IsOfSize(8).Now();
      Verify.That(target.OppCards).IsOfSize(8).Now();
      Verify.That(target.LogoutButton).IsNotNull().Now();
      Verify.That(target.HasBattleEnded).IsFalse().Now();
      Verify.That(target.OppInfo).IsNotNull().Now();
      Verify.That(target.MyInfo).IsNotNull().Now();
      Verify.That(target.MyCards)
            .IsTrueForAll(
              card => card.Selectable,
              "Player's cards should be selectable at the beginning.")
             .Now();
      Verify.That(target.OppCards)
            .IsFalseForAll(
              card => card.Selectable,
              "Opponent's cards should never be selectable.")
             .Now();
    }
    ...
    /// </summary>
    [Test]
    [Ignore("This test is too fragile, method is too long.")]
    public void CanPlayAWholeTurn()
    {
      ...
      Verify.That(target.MyCards.ElementAt(0).Selectable)
          .IsFalse("An already selected card, should no longer be selectable")
          .Now();
      ...
      Verify.That(target.MyCards.Skip(1))
          .IsTrueForAll(
            card => card.Selectable,
            "All cards not selected should still be selectable.")
          .Now();
      ...
      /*Verify.That(target.IsWaitingForOpponent).IsTrue()
          .Now();*/
      // NOTE: For some reason I am assuming card state is saved
      //       but on each CreateInstance() call, card state is lost.
      Verify.That(target.MyCards.ElementAt(1))
          .Member(card => card.Selectable)
            (_ => _.IsFalse("The chosen card should not be selectable."))
          .Now();
